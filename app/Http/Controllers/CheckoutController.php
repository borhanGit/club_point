...
class CheckoutController extends Controller
{
    ...
    public function checkout_done($combined_order_id, $payment,$total)
    {

        $combined_order = CombinedOrder::findOrFail($combined_order_id);


        $wallet_blance = ConvertWallet::where('user_id',Auth::user()->id)->where('status',0)->sum('money');
        $testData = ConvertWallet::where('user_id',Auth::user()->id)->where('status',0)->whereNotNull('product_id')->get();
        foreach($testData as $data){
            ConvertWallet::where('id',$data->id)->update(['status'=> 1]);
        }
        $new_wallet = $wallet_blance - $total;
        ...
        $wallet = new ConvertWallet();
        $wallet->user_id = Auth::user()->id;
        $wallet->money = $new_wallet;
        $wallet->status = 2;
        $wallet->save();
